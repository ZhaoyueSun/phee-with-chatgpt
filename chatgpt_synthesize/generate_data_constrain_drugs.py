...
def main():
    ...
    with open("chatgpt_synthesize/templates/synthesize_instruction4_constrain_drug.json", 'r') as f:
    ...
    drug_list = get_drug_list(train_data)
    neg_drugs = ['drug', 'drugs', 'n/a', 'N/A']
    for neg in neg_drugs:
        if neg in drug_list:
            drug_list.remove(neg)
    ...
    output_folder = "chatgpt_synthesize/output_3_constrain_drug/"
    ...
    constraint_file = os.path.join(output_folder,"constraint_drugs.json")
    ...
    for sample_id, sample in tqdm.tqdm(train_data.items()):
        ...
        try:
            ...
            sampled_drug = random.sample(drug_list, 1)[0]
            constrain_drugs[sample_id] = sampled_drug
            ...
            syn_prompt = synthesize_instruction.replace("<sentence>", sentence).replace("<output>", sample['answer']).replace("<CONST_DRUG>", sampled_drug)

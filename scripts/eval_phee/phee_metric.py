    ...
    for arg_type in cls_stats:
        for stat_name in cls_stats[arg_type]:
            ...
            if 'Trigger' in arg_type: # trigger
                ...
                warnings.warn("Unknown arg type %s:"%arg_type)
...
def _get_stats(instances):
    ...
    for instance in instances:
        ...
        golds.sort()
        if preds == golds:
            stats[ent_type]["consistent_args"] += 1
        elif not preds and golds:
            stats[ent_type]["miss_args"] += 1
        elif not golds and preds:
            stats[ent_type]["reluctant_args"] += 1
        else:
            stats[ent_type]["inconsistent_args"] += 1
...
def compute_metric(instances):
    ...
    for arg_type in cls_keys:
        ...
        for t in ["consistent_args", "inconsistent_args", "miss_args", "reluctant_args"]:
            rst_title = "_".join(['CLS', arg_type, t.upper()])
            results[rst_title] = cls_stats[arg_type][t]

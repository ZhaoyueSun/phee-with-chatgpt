...
def read_pred_results(pred_folder, gold_instances):
    ...
    for gold in gold_instances:
        ...
        if not os.path.exists(pred_file):
            ...
            print("Predict file not found: ", pred_file)
            continue
        with open(pred_file, 'r') as f:
            try:
                preds = json.load(f) 
                pred_evts = json.loads(preds["answer"])
            except:
                print("illegal format file: %s"%pred_file)
                break_format += 1
                pred_evts = []
        ...
        try:
            ...
            print("break argument format: %s"%pred_file)
            break_format += 1
            instance = defaultdict(list)
...
def main():
    ...
    pred_folder = "chatgpt_few_shot/output/TreeKernelW-Type_demo_5/%s"%split
    out_file = "chatgpt_few_shot/output/TreeKernelW-Type_demo_5/%s_result.json"%split

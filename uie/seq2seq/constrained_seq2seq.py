...
class ConstraintSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def prediction_step(
        ...
        ### modify to fit transformer 4.25.1
        ...
        gen_kwargs["prefix_allowed_tokens_fn"] = prefix_allowed_tokens_fn if self.constraint_decoder else None
        ### 
        ...
        with torch.no_grad():
            # if self.use_amp or self.use_cuda_amp:
            #     with autocast():
            #         outputs = model(**inputs)
            # else:
            #     outputs = model(**inputs)
            with self.compute_loss_context_manager(): ### modify to fit transformer 4.25.1
                outputs = model(**inputs)
            if has_labels:

...
class SpotAsocConstraintDecoder(ConstraintDecoder):
    ...
    def get_state_valid_tokens(self, src_sentence, tgt_generated):
        ...
        if self.tokenizer.sep_token_id in src_sentence:
            src_sentence = src_sentence[src_sentence.index(self.tokenizer.sep_token_id) + 1:]
            if self.tokenizer.sep_token_id in src_sentence:
                src_sentence = src_sentence[:src_sentence.index(self.tokenizer.sep_token_id)]
    ...
    def search_prefix_tree(self, generated: List[str], prefix_tree: Dict,
        ...
        tree = prefix_tree
        for index, token in enumerate(generated):
            if token in tree:  # work around for generated invalid tokens
                tree = tree[token]
                is_tree_end = len(tree) == 1 and self.tree_end in tree
            else:
                is_tree_end = True

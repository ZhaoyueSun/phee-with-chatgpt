...
from uie.extraction.noiser.ordered_spot_asoc_noiser import SpotAsocNoiser
...
class DynamicSSIGenerator():
    ...
    def convert_prefix(candidates, prompt, mapper, ordered_prompt=True):
        ...
        if ordered_prompt:
            candidate_sorted = sorted([(candidate, index) for index, candidate in enumerate(candidates)], key=lambda x:list(mapper.keys()).index(x[0]))

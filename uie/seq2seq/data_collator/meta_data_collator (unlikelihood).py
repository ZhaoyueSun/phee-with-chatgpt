...
from uie.extraction.constraint_utils import *
from uie.seq2seq.constraint_decoder.constraint_decoder import ConstraintDecoder
...
class DataCollatorForMetaSeq2Seq:
    ...
    decoder_attention_mask_context: Optional[bool] = False
    unlikelihood_mask_strategy: Optional[str] = None
    constraint_decoder: Optional[ConstraintDecoder] = None

    
    def get_unlikelihood_mask(self, labels, input_ids, constraint_strategy='vocab'):
        input_tokens = self.tokenizer.convert_ids_to_tokens(input_ids)
  
        if constraint_strategy == 'vocab':
            valid_tokens = get_valid_vocab(input_tokens)
            valid_ids = self.tokenizer.convert_tokens_to_ids(valid_tokens)
            full_vocab_size = self.tokenizer.vocab_size + len(self.tokenizer.get_added_vocab())
            token_mask = np.zeros(full_vocab_size)
            np.put(token_mask, valid_ids, 1)
            token_mask = 1 - token_mask
            output_unlikelihood_masks = np.array([token_mask])
            output_unlikelihood_masks = np.repeat(output_unlikelihood_masks, len(labels), axis=0)
        elif constraint_strategy == 'struct':
            if self.constraint_decoder is None:
                raise Exception('Need define constraint_decoder for structral unlikelihood mask!')
            tgt_generated = [self.tokenizer.pad_token_id]
            output_unlikelihood_masks = []
            for tid in labels:
                valid_ids = self.constraint_decoder.get_state_valid_tokens(input_ids, tgt_generated)
                full_vocab_size = self.tokenizer.vocab_size + len(self.tokenizer.get_added_vocab())
                token_mask = np.zeros(full_vocab_size)
                np.put(token_mask, valid_ids, 1)
                token_mask = 1 - token_mask
                output_unlikelihood_masks.append(token_mask)
                tgt_generated += [tid]
            output_unlikelihood_masks = np.array(output_unlikelihood_masks)
        elif constraint_strategy == 'phee_arg_rule':
            # rule: for main_args: unlikely to be repeated with other main_args
            # for sub_args: unlikely to be tokens not in main_args
            if self.constraint_decoder is None:
                raise Exception('Need define constraint_decoder for structral unlikelihood mask!')

            main_to_sub = {
                'subject':["subject disorder", "race", "age", "population", "gender"],
                'treatment':["treatment disorder", "time elapsed", "route", "duration", 
                "frequency", "combination drug", "dosage", "drug"],
                'effect': []
            }
            sub_to_main = {}
            for main_arg in main_to_sub:
                for sub_arg in main_to_sub[main_arg]:
                    sub_to_main[sub_arg] = main_arg
            
            tgt_generated = [self.tokenizer.pad_token_id]
            output_unlikelihood_masks = []
            for tid in labels:
                state, _ = self.constraint_decoder.check_state(tgt_generated)
                valid_ids = self.constraint_decoder.get_state_valid_tokens(input_ids, tgt_generated)
                if state == 'generate_role_text':
                    generated_tokens = self.tokenizer.convert_ids_to_tokens(tgt_generated)
                    generated_roles, cur_role_type = parse_generated_seq(generated_tokens)
                    if cur_role_type is not None:
                        cur_role_type = self.tokenizer.decode(self.tokenizer.convert_tokens_to_ids(cur_role_type.split()))
                        generated_role_dict = defaultdict(list)
                        for role, text in generated_roles:
                            role = self.tokenizer.decode(self.tokenizer.convert_tokens_to_ids(role.split()))
                            generated_role_dict[role] += text.split()

                        if cur_role_type in main_to_sub: # main_args:
                            unlikely_tokens = []
                            for role in generated_role_dict:
                                if role in main_to_sub and role != cur_role_type:
                                    unlikely_tokens += generated_role_dict[role]
                            unlikely_tokens = list(set(unlikely_tokens))
                            unlikely_ids = self.tokenizer.convert_tokens_to_ids(unlikely_tokens)
                            valid_ids = list(set(valid_ids) - set(unlikely_ids))
                        elif cur_role_type in sub_to_main: # sub_args:
                            main_arg = sub_to_main[cur_role_type]
                            if main_arg in generated_role_dict:
                                main_arg_tokens = generated_role_dict[main_arg]
                                main_arg_ids = self.tokenizer.convert_tokens_to_ids(main_arg_tokens)
                                valid_ids = list(set(valid_ids) & set(main_arg_ids)) + [self.tokenizer.convert_tokens_to_ids(constants.type_end)]
                        else:
                            Warning.warn("unknown argument type: %s !"%cur_role_type)
        ...
        return output_unlikelihood_masks
    ...
    def __call__(self, features):
        ...
        for feature in features:
            ...
            feature['attention_mask'] = [1] * len(prefix)
            if self.decoder_attention_mask_context:
                feature['decoder_encoder_attention_mask'] = [1] * len(prefix)
            
            if len(prefix) < self.max_prefix_length:
                prefix += [self.tokenizer.pad_token_id for _ in range((self.max_prefix_length - len(prefix)))]
                feature['attention_mask'] += [0] * (self.max_prefix_length - len(prefix))
                if self.decoder_attention_mask_context:
                    feature['decoder_encoder_attention_mask'] += [0] * (self.max_prefix_length - len(prefix))

            feature['attention_mask'] += [1] * (len(feature['input_ids']) + 1)
            if self.decoder_attention_mask_context:
                query_start_id = feature['input_ids'].index(self.tokenizer.sep_token_id)
                query_end_id = feature['input_ids'].index(self.tokenizer.sep_token_id, query_start_id+1)
                input_cross_attn_mask = [0] * (len(feature['input_ids']))
                input_cross_attn_mask[query_start_id+1:query_end_id] = [1] * (len(input_cross_attn_mask[query_start_id+1:query_end_id]))
                feature['decoder_encoder_attention_mask'] += [1]
                feature['decoder_encoder_attention_mask'] += input_cross_attn_mask
            ...
            if self.decoder_attention_mask_context:
                feature['decoder_encoder_attention_mask'] = feature['decoder_encoder_attention_mask'][:self.max_length]
            
            # feature['attention_mask'] = [1] * len(feature['input_ids'])
            if self.unlikelihood_mask_strategy:
                feature['unlikelihood_mask'] = self.get_unlikelihood_mask(feature['labels'], feature['input_ids'], self.unlikelihood_mask_strategy)
        ...
        if labels is not None:
            ...
            # pad unlikelihood_mask
            if self.unlikelihood_mask_strategy:
                for feature in features:
                    remainder = np.array([np.zeros_like(feature["unlikelihood_mask"][0])])
                    remainder = np.repeat(remainder, (max_label_length - len(feature["unlikelihood_mask"])), axis = 0)
                    feature["unlikelihood_mask"] = np.concatenate((feature["unlikelihood_mask"], remainder), axis=0)


        # We have to pad the `decoder_encoder_attention_mask` before calling `tokenizer.pad` as this method won't pad them and needs them of the
        # same length to return tensors.
        input_ids = [feature["input_ids"] for feature in features] if "input_ids" in features[0].keys() else None
        if self.decoder_attention_mask_context:
            max_input_length = max(len(_input_ids) for _input_ids in input_ids)
            padding_side = self.tokenizer.padding_side
            for feature in features:
                remainder = [0] * (max_input_length - len(feature["decoder_encoder_attention_mask"]))
                feature["decoder_encoder_attention_mask"] = (
                    feature["decoder_encoder_attention_mask"] + remainder if padding_side == "right" else remainder + feature["decoder_encoder_attention_mask"]
                )
